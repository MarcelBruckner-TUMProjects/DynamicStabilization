...
namespace dynamic_stabilization {
	namespace stabilization {
		 ...
		 */
		class DynamicStabilizationBase : public dynamic_stabilization::utils::TimeMeasurable {
		private:
			/**
		...
		protected:
			 ...
			 */
			std::shared_ptr<dynamic_stabilization::stabilization::detection::FeatureDetectionBase> frameFeatureDetector,
				referenceFeatureDetector;

			/**
			 * Feature matcher to match the frame and reference frame stabilization::features.
			 */
			std::shared_ptr<dynamic_stabilization::stabilization::matching::FeatureMatchingBase> matcher;

			/**
			 * Warps the frame based on the given matches.
			 */
			std::shared_ptr<dynamic_stabilization::stabilization::FrameWarping> warper;
		...
		public:
			 ...
			 */
			const std::shared_ptr<dynamic_stabilization::stabilization::FrameWarping> &getWarper() const;

			/**
			 * @get The background segmentor used to mask the writeFrames.
			 */
			const std::shared_ptr<dynamic_stabilization::stabilization::segmentation::BackgroundSegmentionBase> &
			getSegmentor() const;

			/**
			 * @get The feature detector for the current frame.
			 */
			const std::shared_ptr<dynamic_stabilization::stabilization::detection::FeatureDetectionBase> &
			getFrameFeatureDetector() const;

			/**
			 * @get The matcher used for matching the frame and reference frame.
			 */
			const std::shared_ptr<dynamic_stabilization::stabilization::matching::FeatureMatchingBase> &getMatcher() const;
